...
namespace tehtävä_18_pudotusvalikot
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            oppiCB.DataSource = oppilaitos;
            oppiCB.DisplayMember = "Onimi";
        ...
        private void oppiCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            string viite = oppilaitos.Rows[oppiCB.SelectedIndex]["OID"].ToString();
            kauLB.Text = oppilaitos.Rows[oppiCB.SelectedIndex]["OKatuosoite"].ToString();
            osLB.Text = oppilaitos.Rows[oppiCB.SelectedIndex]["OPostinumero"].ToString();
            postLB.Text = oppilaitos.Rows[oppiCB.SelectedIndex]["OPostitoimipaikka"].ToString();
            puhLB.Text = oppilaitos.Rows[oppiCB.SelectedIndex]["OPuhelin"].ToString();

            yhteys = vastuuHenkilot.Select("OID =" + viite).CopyToDataTable();
            henkCB.DataSource = yhteys;
            henkCB.DisplayMember= "VNimi";
        }

        private void henkCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            titteliLB.Text = yhteys.Rows[henkCB.SelectedIndex]["VTitteli"].ToString();
            ammaLB.Text = yhteys.Rows[henkCB.SelectedIndex]["VOsasto"].ToString();
            sahLB.Text = yhteys.Rows[henkCB.SelectedIndex]["VSahkoposti"].ToString();
            tpuhLB.Text = yhteys.Rows[henkCB.SelectedIndex]["VPuhelin"].ToString();
        }
    }
}